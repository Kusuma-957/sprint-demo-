...
public class RoomAmenityService {
    ...
    /**
     * Creates a Room-Amenity mapping.
     * - 404 if Room or Amenity doesn't exist (ResourceNotFoundException)
    ...
    public RoomAmenityResponseDTO createMapping(RoomAmenityCreateDTO dto) {
        ...
        // Prevent duplicate mapping
        boolean mappingExists = roomAmenityRepository.existsMapping(room.getRoomId(), amenity.getAmenityId());
        if (mappingExists) {
            // Maps to 409 with code ADDFAILS in GlobalExceptionHandler
            throw new ConflictException("Room amenity already exist");
        }
